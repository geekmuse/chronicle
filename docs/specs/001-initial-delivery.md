...
```crontab
@reboot SSH_AUTH_SOCK=$(find /private/tmp/com.apple.launchd.* -name Listeners -user $(whoami) -type s 2>/dev/null | head -1) /usr/local/bin/chronicle sync --quiet  # chronicle-sync
*/5 * * * * SSH_AUTH_SOCK=$(find /private/tmp/com.apple.launchd.* -name Listeners -user $(whoami) -type s 2>/dev/null | head -1) /usr/local/bin/chronicle sync --quiet  # chronicle-sync
```

Each entry is prefixed with an `SSH_AUTH_SOCK` discovery snippet so that the `ssh_key_from_agent()` credential callback can reach the user's SSH agent from the minimal cron environment. On macOS this scans `/private/tmp/com.apple.launchd.*/Listeners` for a socket owned by the current user (neither `launchctl getenv` nor `launchctl asuser` work from cron's system bootstrap context); on Linux it falls back to the systemd user socket at `/run/user/<uid>/ssh-agent.socket`.
